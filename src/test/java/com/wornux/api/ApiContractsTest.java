...
class ApiContractsTest extends AbstractRestController {
    ...
    @Test void responseFactoriesAndControllerHelpersPreserveTheirContracts() {
        ...
        assertThat(created("made", URI.create("/x/1"), 1).getHeaders().getLocation()).hasPath("/x/1");
    ...
    @Test void configurationFactoriesExposeExpectedJsonAndOpenApiContracts() throws Exception {
    ...
    @Test void exceptionHandlerMapsValidationAndSecurityFailures() {
        ...
        assertThat(handler.authentication(new BadCredentialsException("bad")).getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
        assertThat(handler.accessDenied(new AccessDeniedException("no")).getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);
        assertThat(handler.runtime(new RuntimeException("boom")).getBody().message()).isEqualTo("Unexpected API error.");
    }

    @Test void productErrorsSelectNotFoundConflictAndBadRequestWithoutCaseSensitivity() {
        var handler = new RestExceptionHandler();
        assertThat(handler.product(new ProductException("Product NOT FOUND")).getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(handler.product(new ProductException("SKU already exists")).getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(handler.product(new ProductException("Updated by another user")).getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(handler.product(new ProductException("invalid")).getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
