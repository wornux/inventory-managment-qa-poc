...
class AuditBehaviorTest {
    ...
    @Test
    void currentUsernameRejectsAbsentUnauthenticatedAnonymousAndBlankIdentities() {
        ...
        SecurityContextHolder.getContext()
                .setAuthentication(UsernamePasswordAuthenticationToken.unauthenticated("u", "p"));

        assertThat(CurrentUserUtils.currentUsername()).isEqualTo(CurrentUserUtils.ANONYMOUS);

        SecurityContextHolder.getContext()
                .setAuthentication(
                        new AnonymousAuthenticationToken("k", "anon", AuthorityUtils.createAuthorityList("A")));
    ...
    @Test
    void authenticatedNameFeedsAuditorAndRevision() {
        SecurityContextHolder.getContext()
                .setAuthentication(
                        new UsernamePasswordAuthenticationToken("alice", "p", AuthorityUtils.NO_AUTHORITIES));
    ...
    @Test
    void auditableAliasesExposeDatesAndAuditProperties() {
        class Item extends Auditable {}
        var item = new Item();
        var created = Instant.EPOCH;
        var modified = Instant.MAX;
        item.setCreatedBy("a");
        item.setLastModifiedBy("b");
        item.setCreatedDate(created);
        item.setLastModifiedDate(modified);
