...
class UiBehaviorTest {
    ...
    @Test void layoutShowsOnlyAllowedNavigationAndAuthenticatedUsername() {
        ...
        var text = descendants(layout).map(Component::getElement).map(e -> e.getText()).toList();
        assertThat(text).contains("Products", "alice", "Signed in");
        assertThat(text).doesNotContain("Categories", "Users", "No modules available");
    }

    @Test void layoutShowsEmptyStateAndFallbackUsernameWhenNothingIsAllowed() {
        var layout = new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class));
        assertThat(descendants(layout).filter(Span.class::isInstance).map(c -> c.getElement().getText()))
                .contains("No modules available", "User");
    }

    @Test void layoutCanShowAdministrationWithoutInventory() {
        var access = mock(UiAccessService.class);
    ...
    @Test void layoutUsesAuthenticationNameAndFallsBackWhenItIsBlank() {
        var authentication = mock(Authentication.class);
        when(authentication.getPrincipal()).thenReturn(new Object());
        when(authentication.getName()).thenReturn("bob");
        SecurityContextHolder.getContext().setAuthentication(authentication);
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("bob");
        when(authentication.getName()).thenReturn(" ");
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("User");
        when(authentication.getName()).thenReturn(null);
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("User");
    }

    @Test void layoutUsesPreferredOidcUsernameWithIdentityNameAsFallback() {
        var authentication = mock(Authentication.class);
        ...
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("alice");
        when(principal.getClaimAsString("preferred_username")).thenReturn(null);
        when(principal.getName()).thenReturn("subject");
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("subject");
        when(principal.getClaimAsString("preferred_username")).thenReturn(" ");
        assertThat(textOf(new MainLayout(mock(AuthenticationContext.class), mock(UiAccessService.class)))).contains("subject");
    }

    @Test void logoutButtonInvokesAuthenticationContext() {
        var authentication = mock(AuthenticationContext.class);
        var layout = new MainLayout(authentication, mock(UiAccessService.class));
        descendants(layout).filter(Button.class::isInstance).map(Button.class::cast)
                .filter(button -> "Sign out".equals(button.getText())).findFirst().orElseThrow().click();
        verify(authentication).logout();
    }

    @Test void protectedRouteReroutesWithoutPermissionAndAllowsWithPermission() {
        var access = mock(UiAccessService.class);
        var layout = new MainLayout(mock(AuthenticationContext.class), access);
