...
class CategoryServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new CategoryService(categories, products, authorization);
    }
    ...
    @Test
    void getReportsMissingCategory() {
    ...
    @Test
    void createNormalizesAndRejectsDuplicateNames() {
    ...
    @Test
    void updateEnforcesExistenceVersionAndUniqueName() {
        Category category = new Category("Old", null);
        when(categories.findById(1L)).thenReturn(Optional.of(category));

        assertThatThrownBy(() -> service.update(1L, request("New", null, true, 2L)))
                .hasMessageContaining("another user");
        assertThatThrownBy(() -> service.update(9L, request("New", null, true, null)))
                .hasMessage("Category was not found.");
        when(categories.existsByNameIgnoreCaseAndIdNot("New", 1L)).thenReturn(true);

        assertThatThrownBy(() -> service.update(1L, request(" New ", null, true, null)))
                .hasMessageContaining("already exists");
        ...
        assertThat(service.update(1L, request("New", null, true, null)).getDescription())
                .isNull();
    }

    @Test
    void deactivateAndCapabilitiesReflectAuthorization() {
        Category category = new Category("Tools", null);
        when(categories.findById(1L)).thenReturn(Optional.of(category));
